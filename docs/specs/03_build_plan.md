...
| D-3 | Severity scoring methodology (CVSS-aligned vs. custom) | `00` §13; `02` §5 | 7 | **Resolved 2026-08-10; BUILT Unit 7** — CVSS 3.1 base score + vector per finding (ADR-D3); `parseSeverity`/`deriveSeverityBand` derive the qualitative band at read time. Validated against real findings in Unit 8 |
| D-4 | OOB listener timeout policy (+ late-callback handling) | `00` §13; `02` §3.2, §8; `01` §12 | 5 | **Resolved 2026-08-10; BUILT Unit 5** — 5-min wait (config) via the `sweepOobTimeouts` resolver; late callback = audit note, never auto-added to a closed report (ADR-D4). Sweep-driven single-resume mechanism recorded as **ADR-32** |
...
| D-12 | LLM daily spend ceilings: global + per-user hard-stop values (mechanism confirmed, ADR-21) | `02` §3.2, §7, §8 | 3 | **Open (values only) — mechanism + defaults BUILT (Unit 3)**: `llm_calls` ledger + `evaluateDailySpend`/`DEFAULT_DAILY_SPEND_CEILINGS` (global 5 / user 1 credits/day, conservative), raised once Unit 8 measures real per-scan cost. BYOK caveat: `usage.cost≈0` under BYOK, so the credit cap is inactive there (Google free-tier limits apply) — accepted for v1 |
| D-13 | JWT confusion verification signal (auth-state oracle) | `00` §9; `02` §4.4 | 5 | **Method resolved 2026-08-10; gate BUILT Unit 5** (`verifyJwt`, ADR-D13); numeric thresholds still calibrated on the labs (Unit 8, external) |
| D-14 | Injection verification signal (error / dose-response time-based / boolean-diff) | `00` §9; `02` §4.4 | 5 | **Method resolved 2026-08-10; gate BUILT Unit 5** (`verifyInjection` error + dose-response, ADR-D14); boolean-differential deferred (additive); thresholds calibrated on the labs (Unit 8, external) |
| D-15 | IDOR verification signal (labeled cross-session ownership proof) | `00` §9; `02` §4.4 | 5 | **Method resolved 2026-08-10; gate BUILT Unit 5** (`verifyIdor` with self/absent controls, ADR-D15); thresholds calibrated on the labs (Unit 8, external) |
| D-16 | SSRF verification signal (blind = OOB/ADR-09; non-blind canary) | `00` §9; `02` §3.2, §4.4 | 5 | **Method resolved 2026-08-10; gate BUILT Unit 5** (`verifySsrf` correlated-OOB, ADR-D16/ADR-32); non-blind canary deferred (additive); thresholds calibrated on the labs (Unit 8, external) |
