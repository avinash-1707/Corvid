...
| Unit 3 — Agent Core (perceive → hypothesize → plan) | Not started | 2026-08-10 | Owns D-10, D-12. LLM via OpenRouter (ADR-23), never verifies; spend cap/kill-switch (ADR-21) |
| Unit 4 — MCP Tool Servers (the testers) | Not started | 2026-08-10 | Owns D-2 (D-1 resolved: analyst creds). `http.send` enforces path scope (ADR-24) + sequential (ADR-25); JWT/injection/SSRF/IDOR testers |
